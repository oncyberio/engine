...
float getSpotAttenuation( const in float coneCosine, const in float penumbraCosine, const in float angleCosine ) {
	...
	void getPointLightInfo( const in PointLight pointLight, const in GeometricContext geometry, out IncidentLight light ) {
		...
		vec3 lVector = getPosition(pointLight.position) - geometry.position;

		light.direction = normalize( lVector );

		float lightDistance = length( lVector );

		// float pDistance = pointLight.distance * smoothstep(40.0, 60.0, abs(geometry.position.z))

		// pointLight.distance *= smoothstep(40.0, 60.0, abs(geometry.position.z));
...
void main() {
	...
	float v2mix = min(fresnelFactor, smoothstep(0.0, 50.0, vViewPosition.z ));

	gl_FragColor.rgb = gl_FragColor.rgb * vec3(1.0 - v2mix) + vec3(gColor * (1.0 -gridForce) * viewWorldDir.r * viewWorldDir.r ) * smoothstep(0.5, 1.0, sin(3.1416 * mod(timer * 0.05 + 1.2, 1.0) ) );
