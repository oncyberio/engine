...
import { MeshBasicMaterial, MeshStandardMaterial } from "engine/xtend";

import { BITMAP_SUPPORT, IS_VR } from "engine/constants";
    ...
    unregister(callback) {
        if (this.pluginCallbacks.indexOf(callback) !== -1) {
            this.pluginCallbacks.splice(
                this.pluginCallbacks.indexOf(callback),
                1,
            );
        }
...
class GLTFLightsExtension {
    ...
    _loadLight(lightIndex) {
        ...
        const extensions =
            (json.extensions && json.extensions[this.name]) || {};
...
class GLTFMaterialsSpecularExtension {
    ...
    extendMaterialParams(materialIndex, materialParams) {
        ...
        materialParams.specularIntensity =
            extension.specularFactor !== undefined
...
class GLTFParser {
    constructor(json = {}, options = {}) {
        ...
        if (BITMAP_SUPPORT) {
            this.textureLoaderBitmap = new ImageBitmapLoader(
                this.options.manager,
            );
            this.textureLoaderBitmap.setCrossOrigin(this.options.crossOrigin);
            this.textureLoaderBitmap.setRequestHeader(
                this.options.requestHeader,
            );
        }

        this.textureLoaderClassic = new TextureLoader(this.options.manager);
        this.textureLoaderClassic.setCrossOrigin(this.options.crossOrigin);
        this.textureLoaderClassic.setRequestHeader(this.options.requestHeader);
    ...
    getLoader() {
        let loader = this.textureLoaderClassic;

        if (BITMAP_SUPPORT && IS_VR == false) {
            loader = this.textureLoaderBitmap;
        }
        ...
        const promise = this.loadImageSource(sourceIndex, loader)
            .then(function (texture) {
                ...
                texture.minFilter =
                    ...
                    LinearMipmapLinearFilter;
                texture.wrapS =
                    WEBGL_WRAPPINGS[sampler.wrapS] || RepeatWrapping;
                texture.wrapT =
                    WEBGL_WRAPPINGS[sampler.wrapT] || RepeatWrapping;
    ...
    loadImageSource(sourceIndex, loader) {
        ...
        if(__BUILD_TARGET__ === 'node') {
            ...
            return Promise.resolve(this._emptyTexture);
        }
        ...
        const promise = Promise.resolve(sourceURI)
            ...
            .catch(function (error) {
                console.error(
                    "THREE.GLTFLoader/loadImageSource: Couldn't load texture",
                    sourceURI,
                );
                throw error;
            });
    ...
    assignTexture(materialParams, mapName, mapDef, colorSpace) {
        ...
        return this.getDependency("texture", mapDef.index).then(
            function (texture) {
    ...
    loadMaterial(materialIndex) {
        ...
        if (materialExtensions[EXTENSIONS.KHR_MATERIALS_UNLIT]) {
            ...
            materialType = this._invokeOne(function (ext) {
                return (
                    ext.getMaterialType && ext.getMaterialType(materialIndex)
                );
            });
    ...
    _loadNodeShallow(nodeIndex) {
        ...
        parser
            ._invokeAll(function (ext) {
                return (
                    ext.createNodeAttachment &&
                    ext.createNodeAttachment(nodeIndex)
                );
            })
        ...
        this.nodeCache[nodeIndex] = Promise.all(pending).then(
            function (objects) {
                let node;
