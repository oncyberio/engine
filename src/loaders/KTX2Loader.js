...
class KTX2Loader extends Loader {
    ...
    detectSupport(renderer) {
        this.workerConfig = {
            astcSupported: renderer.extensions.has(
                "WEBGL_compressed_texture_astc",
            ),
            etc1Supported: renderer.extensions.has(
                "WEBGL_compressed_texture_etc1",
            ),
            etc2Supported: renderer.extensions.has(
                "WEBGL_compressed_texture_etc",
            ),
            dxtSupported: renderer.extensions.has(
                "WEBGL_compressed_texture_s3tc",
            ),
            bptcSupported: renderer.extensions.has(
        ...
        if (!this.transcoderPending) {
            ...
            this.transcoderPending = Promise.all([
                ...
                const body = [
                    "/* constants */",
                    "let _EngineFormat = " +
                        JSON.stringify(KTX2Loader.EngineFormat),
                    "let _TranscoderFormat = " +
                        JSON.stringify(KTX2Loader.TranscoderFormat),
                    "let _BasisFormat = " +
                        JSON.stringify(KTX2Loader.BasisFormat),
                    "/* basis_transcoder.js */",
                    jsContent,
                    "/* worker */",
    ...
    _createTextureFrom(transcodeResult, container) {
        ...
        texture.colorSpace =
            dfdTransferFn === KHR_DF_TRANSFER_SRGB
                ? SRGBColorSpace
                : LinearEncoding;
        texture.premultiplyAlpha = !!(
            dfdFlags & KHR_DF_FLAG_ALPHA_PREMULTIPLIED
        );
    ...
    function transcode(buffer) {
        ...
        for (let mip = 0; mip < levels; mip++) {
            ...
            for (let layer = 0; layer < layers; layer++) {
                ...
                mipWidth =
                    levelInfo.origWidth < 4
                        ? levelInfo.origWidth
                        : levelInfo.width;
                mipHeight =
                    levelInfo.origHeight < 4
                        ? levelInfo.origHeight
                        : levelInfo.height;
...
async function createDataTexture(container) {
    ...
    texture.colorSpace = ENCODING_MAP[vkFormat] || LinearEncoding;
