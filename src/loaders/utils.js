 ...
 */
function mergeBufferGeometries(geometries, useGroups = false, opts = {}) {
    const forceList = opts.forceList || [];

    const isIndexed = geometries[0].index !== null;

    const allAttributes = new Set(Object.keys(geometries[0].attributes));

    let attributesUsed = new Set();

    if (forceList.length > 0) {
        forceList.forEach((item) => {
            if(allAttributes.has(item)) {
                attributesUsed.add(item);
            }
        });
    }
    else {
        attributesUsed = allAttributes;
    }
    ...
    for (let i = 0; i < geometries.length; ++i) {
        ...
        if(opts.ignoreMorphTargets !== true) {
    ...
    for (const name in attributes) {
        // Only merge if forceList is empty or the attribute name is in forceList
        if (forceList.length === 0 || forceList.includes(name)) {
		...
		if ( attribute.isInterleavedBufferAttribute ) {
			...
			for ( let j = 0, l = attribute.count; j < l; j ++ ) {
				...
				for ( let c = 0; c < itemSize; c ++ ) {
					...
					const value = getComponent( attribute, j, c );
					setComponent( result, j + tupleOffset, c, value );
...
function getComponent( attribute, index, component ) {
    ...
    if ( attribute.normalized ) value = denormalize( value, attribute.array );
...
function setComponent( attribute, index, component, value ) {
    ...
    if ( attribute.normalized ) value = normalize( value, attribute.array );
    ...
    for (let i = 0, l = attributeNames.length; i < l; i++) {
        ...
        if (morphAttr) {
            tmpMorphAttributes[name] = new BufferAttribute(
                new morphAttr.array.constructor(
