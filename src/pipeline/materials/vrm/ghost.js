...
import PreVertex from "./shader/ghost/pre.vertex.glsl";

import MainVertex from "./shader/ghost/main.vertex.glsl";

import SuffVertex from "./shader/ghost/suff.vertex.glsl";

import PreFrag from "./shader/ghost/pre.frag.glsl";

import MainFrag from "./shader/ghost/main.frag.glsl";

import SuffFrag from "./shader/ghost/suff.frag.glsl";

export default class VRMInstancedGhostMaterial extends InstancedBasic {
  constructor(options, params) {
    ...
    let opts = {
      uniforms: {
        rimPower: { value: params?.uniforms?.rimPower?.value ?? 1.0 },

        minAlpha: { value: params?.uniforms?.minAlpha?.value ?? 0.05 },
