...
class KTX2Loader extends Loader {
  ...
  _createTextureFrom(transcodeResult, container) {
    const { faces, width, height, format, type, error, dfdFlags } =
      transcodeResult;
    ...
    texture.minFilter =
      faces[0].mipmaps.length === 1 ? LinearFilter : LinearMipmapLinearFilter;
...
async function createRawTexture(container) {
  ...
  for (let levelIndex = 0; levelIndex < container.levels.length; levelIndex++) {
    ...
    const levelDepth = container.pixelDepth
      ? Math.max(1, container.pixelDepth >> levelIndex)
      : 0;
...
function parseColorSpace(container) {
  ...
  if (dfd.colorPrimaries === KHR_DF_PRIMARIES_BT709) {
    ...
    console.warn(
      `THREE.KTX2Loader: Unsupported color primaries, "${dfd.colorPrimaries}"`
    );
    return NoColorSpace;
  }
}
