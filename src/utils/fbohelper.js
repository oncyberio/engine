...
export default class THREEFBOHelper {
  constructor(renderer) {
    ...
    this.grid.setAttribute(
      "style",
      "position: fixed; z-index: 10000; left: 50%; top: 50%; border: 1px solid #000000; transform: translate3d(-50%, -50%, 0 ); box-shadow: 0 0 50px black; display: none"
    );
    ...
    this.grid.addEventListener("wheel", (e) => {
      ...
      this.label.style.transform = `scale(${1 / this.camera.zoom},${
        1 / this.camera.zoom
      })`;
      this.hotspot.style.transform = `scale(${1 / this.camera.zoom},${
        1 / this.camera.zoom
      })`;
    ...
    this.grid.addEventListener("mousemove", (e) => {
      if (dragging) {
        this.offsetX =
          offsetStart.x + (e.clientX - mouseStart.x) / this.camera.zoom;
        this.offsetY =
          offsetStart.y + (e.clientY - mouseStart.y) / this.camera.zoom;
        ...
        const intersects = this.raycaster.intersectObject(
          this.currentObj.quad,
          true
        );

        if (intersects.length > 0) {
          this.readPixel(
            this.fboMap.get(intersects[0].object),
            intersects[0].uv.x,
            intersects[0].uv.y
          );
          this.label.style.display = "block";
  ...
  attach(fbo, name, formatter) {
    ...
    const material = new THREE.MeshBasicMaterial({
      map: fbo.texture == null ? fbo : fbo.texture,
    ...
    li.addEventListener("click", (e) => {
      ...
      if (quad.visible) {
        ...
        this.info.innerHTML = `Width: ${fbo.width} Height: ${
          fbo.height
        }<br/>Format: ${
          formats[fbo.texture ? fbo.texture.format : fbo.format]
        } Type: ${types[fbo.texture ? fbo.texture.type : fbo.type]}`;
      } else {
        this.info.style.display = "none";
  ...
  readPixel(obj, u, v) {
    ...
    types[THREE.HalfFloatType] = Float32Array;
    ...
    if (fbo.isDataTexture == true) {
      const component = fbo.format == 1023 ? 4 : 3;

      const width = fbo.source.data.width;
      const height = fbo.source.data.height;
      ...
      // console.log("total ", width * height * component )
      // console.log(x, y)
      ...
      // console.log(fbo.source.data.data.length)

      pixelBuffer[0] = fbo.source.data.data[index * component].toFixed(4);
      pixelBuffer[1] = fbo.source.data.data[index * component + 1].toFixed(4);
      pixelBuffer[2] = fbo.source.data.data[index * component + 2].toFixed(4);
      pixelBuffer[3] = fbo.source.data.data[index * component + 3].toFixed(4);
      // const rgb = fbo.image.data[]
    ...
    const oy =
      (~~(obj.flipY ? (1 - v) * fbo.height : v * fbo.height) *
        obj.quad.height) /
      fbo.height;
