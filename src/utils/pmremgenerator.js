// src https://github.com/mrdoob/three.js/blob/master/src/extras/PMREMGenerator.js
import {
...
const tempFrustrum = new Frustum();
...
class PMREMGenerator {
   ...
   */
  fromScene(scene, sigma = 0, near = 0.1, far = 100, opts) {
    ...
    this._sceneToCubeUV(scene, near, far, cubeUVRenderTarget, opts);
  ...
  _allocateTargets() {
    ...
    cubeUVRenderTarget.texture.name = "PMREM.CubeUVRenderTarget";
    ...
    if (
      ...
      this._pingPongRenderTarget.texture.name = "PMREM.PingPongRenderTarget";
  ...
  _sceneToCubeUV(scene, near, far, cubeUVRenderTarget, opts = {}) {
    const fov = 90;
    const aspect = 1;
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (opts.sceneOpts != null) {
        cubeCamera.position.set(
          opts.sceneOpts.position.x,
          opts.sceneOpts.position.y,
          opts.sceneOpts.position.z
        );
      }
      ...
      projScreenMatrix.multiplyMatrices(
        cubeCamera.projectionMatrix,
        cubeCamera.matrixWorldInverse
      );

      tempFrustrum.setFromProjectionMatrix(projScreenMatrix);

      cubeCamera.frustum = tempFrustrum;
      ...
      Emitter.emit(Events.PRE_RENDER, scene, cubeCamera);
